...
---------------------------------------------------------
-- PYTHON & STUDY BINDINGS (Customized for VegetaMomochi)
---------------------------------------------------------

-- Run the current Python file (Perfect for your test practice)
vim.keymap.set('n', '<F5>', ':w<CR>:!python3 %<CR>', { desc = 'Save and Run Python' })

-- Select All (Quickly copy or delete your entire script)
...
-- Clear Search Highlights (Press Esc to get rid of yellow search boxes)
vim.keymap.set('n', '<Esc>', ':noh<CR>', { desc = 'Clear search highlights' })

-- Window Splitting (Use these to see two files side-by-side in st)
vim.keymap.set('n', '<leader>v', ':vsplit<CR>', { desc = 'Split screen vertically' })
vim.keymap.set('n', '<leader>h', ':split<CR>', { desc = 'Split screen horizontally' })
...
-- Better Indenting (Keeps the code selected while you move it left/right)
vim.keymap.set('v', '<', '<gv', { desc = 'Indent left and re-select' })
vim.keymap.set('v', '>', '>gv', { desc = 'Indent right and re-select' })

-- Quick Commenting (Useful for testing different Python logic)
-- Note: This requires a commenting plugin which you already have in your config
vim.keymap.set('n', '<C-_>', 'gcc', { remap = true, desc = 'Comment current line' })
vim.keymap.set('v', '<C-_>', 'gc', { remap = true, desc = 'Comment selected block' })
